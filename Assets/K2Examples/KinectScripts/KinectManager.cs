...
//	public bool useOwnThread = false; 
	...
	public static KinectInterop.SensorData sensorData = null;
	...
	// game modes
	public enum GameMode { None, View, Record, Play, Feedback};
	[Tooltip("Indicates game's current state.")]
	public static GameMode gameModeStat = GameMode.None;

	// correction
	public static bool correctAvatarMovement = true;

	// speed
	public static int speed = 0;

	//say narrative text 'start' only once
	public static bool gameStarted = false;
	public static bool gestureDetected = false;

	// display progress
	public static ProgressRadialBehaviour progressUI;
	public static GameObject progressCircle;
	public static GameObject viewButton;
	public static GameObject speedSlider;
	public static GameObject speedText;

	public static StreamWriter skeletonStreamWriter = null;
	public static StreamReader skeletonStreamReader = null;
	public static int currentFrame = 0;
	public static bool writeSkeletonFile = false;
	public static bool readSkeletonFile = false;
	...
	{
		speed = (int)(GameObject.Find("Canvas/SpeedSlider").GetComponent<Slider>().maxValue - GameObject.Find("Canvas/SpeedSlider").GetComponent<Slider>().value);
	}

	public string selectFileFromDialog(string title, string path, string extension)
	{
	...
	public void viewModeButton()
	{
		if (gameModeStat == GameMode.View)
		{
			gameModeStat = GameMode.None;
			readSkeletonFile = false;

			AvatarController.raiseHandVoiceInfo = true;
			AvatarController.showFrameInfo = false;
		}
		else
		{
			AvatarController.raiseHandVoiceInfo = false;
			AvatarController.view_file = selectFileFromDialog("Dance moves folder", AvatarController.root_path + "DanceMoves", "txt");
			if (AvatarController.view_file != null && AvatarController.view_file.Length != 0)
			{
				gameModeStat = GameMode.View;
				AvatarController.showFrameInfo = true;

				AvatarController.framesCount = 0;
				AvatarController.currentFrame = 0;
				AvatarController.frames = AvatarController.loadFrames(AvatarController.view_file, ref AvatarController.framesCount);

				string[] comp = (AvatarController.view_file).Split(Path.DirectorySeparatorChar);
				string skel_file = AvatarController.skeletons_directory + comp[comp.Length - 1];

				readSkeletonFile = true;
				AvatarController.skel_framesCount = 0;
				AvatarController.skel_currentFrame = 0;
				AvatarController.skel_frames = AvatarController.skel_loadFrames(skel_file, ref AvatarController.skel_framesCount);
	...
	public void resetProgram()
	{
		...
		if (gameModeStat == GameMode.Record && AvatarController.streamWriter != null)
		{
...
//		}
    ...
    {
        Rect cameraRect = foregroundCamera ? foregroundCamera.pixelRect : new Rect(0, 0, UnityEngine.Screen.width, UnityEngine.Screen.height);
    ...
    void Awake()
	{
		progressUI = GameObject.Find("ProgressCircle/filler").GetComponent<ProgressRadialBehaviour>();
		progressCircle = GameObject.Find("Canvas/ProgressCircle");
		viewButton = GameObject.Find("Canvas/ViewButton");
		speedSlider = GameObject.Find("Canvas/SpeedSlider");
		speedText = GameObject.Find("Canvas/SpeedText");
		...
		if(kinectInitialized)
		{
			   ...
			   (computeUserMap != UserMapType.None && computeUserMap != UserMapType.RawUserDepth))
	        {
				if(usersMapRect.width == 0 || usersMapRect.height == 0)
				{
					...
					Rect cameraRect = Camera.main != null ? Camera.main.pixelRect : new Rect(0, 0, UnityEngine.Screen.width, UnityEngine.Screen.height);
					...
					//usersMapRect = new Rect(cameraRect.width - displayWidth - 540, cameraRect.height - 100, displayWidth, -displayHeight);
					usersMapRect = new Rect(cameraRect.width - displayWidth - 500, cameraRect.height - 75, displayWidth*2, -displayHeight*1.75f);
				}
...
//					}
	...
	void Update() 
	{
		//can't be in awake(), all structures need to be initialised first
		if (!gameStarted)
        {
			NarrativeText.setText("Say dance or wave your hand when you are ready to start");
			NarrativeText.showInfoText("Say dance or wave your hand when you are ready to start", 4.0f);
			int noPlaybackFiles = Directory.GetFiles(AvatarController.playback_directory).Length;

			//set playback file if already exists
			...
			if(!lateUpdateAvatars)
			{
				foreach (AvatarController controller in avatarControllers)
				{
					...
					if(userId != 0 && dictUserIdToIndex.ContainsKey(userId))
					{
						...
						if ((gameModeStat == GameMode.Play && gestureDetected) || gameModeStat == GameMode.Record)
						{
							controller.UpdateAvatarFromKinect(userId);
						}
					}
					if (gameModeStat == GameMode.View || gameModeStat == GameMode.Play || gameModeStat == GameMode.Feedback)
					{
	...
	void UpdateUserMap(UserMapType userMapType)
    {
		   ...
		   !sensorData.sensorInterface.IsBackgroundRemovalActive())
		{
			...
			if(userMapType != UserMapType.RawUserDepth)
			{
				...
				if(gameModeStat == GameMode.Record)
				{
					for (int i = 0; i < alUserIds.Count; i++)
					{
				...
				if (!writeSkeletonFile && readSkeletonFile)
				{
					AvatarController.DrawSkeleton(usersLblTex2D);
				}
...
//		}
	...
	private void DrawSkeleton(Texture2D aTexture, ref KinectInterop.BodyData bodyData)
	{
		...
		if (writeSkeletonFile)
		{
			currentFrame++;
			skeletonStreamWriter.WriteLine(currentFrame.ToString());
		}
		for (int i = 0; i < jointsCount; i++)
		{
			...
			if(isJointTracked && isParentTracked)
			{
				...
				if(posParent != Vector2.zero && posJoint != Vector2.zero)
				{
					...
					if (writeSkeletonFile)
					{
						skeletonStreamWriter.WriteLine(((int)posParent.x).ToString() + "," + ((int)posParent.y).ToString() + "," +
						((int)posJoint.x) + "," + ((int)posJoint.y).ToString());
					}
