...
{	
	...
	[Tooltip("Whether the avatar's hands will be set to T-pose after playback.")]
	public bool resetToTpose = false;

	[Tooltip("Whether the avatar's position will be set to default after playback.")]
	public bool resetToInitPos = false;

	[Tooltip("Factor used to correct wrong movements on y.")]
	public float yCorrectionFactor = 3.5f;

	[Tooltip("Max time interval (in seconds) that can be recorded.")]
	public int maxRecTime = 60;

	[Tooltip("Specifies whether this avatar is trainer.")]
	...
	// private instance of the PauseMenu
	...
	// variables storing avatar's tip toes
    public GameObject tipToesLeft, tipToesRight;

	// auxiliary files
	public static string root_path = Directory.GetCurrentDirectory() + Path.DirectorySeparatorChar + "Database" + Path.DirectorySeparatorChar;

	public static string playback_directory = root_path + "DanceMoves" + Path.DirectorySeparatorChar;
	public static string playback_file;

	public static string skeletons_directory = root_path + "Skeletons" + Path.DirectorySeparatorChar;
	public static string skeleton_file;

	public static string user_records_directory = root_path + "UserRecords" + Path.DirectorySeparatorChar;
	public static string user_record_file;

	public static string feedback_file;
	public static string record_file;
	public static string view_file;

	public static bool allowFeedback = false;
	public static bool feedbackVoiceInfoOnlyOnce = true;

	//constants
	public static int noReps = 0;
	public static int maxRepetitions = 1;
	private static int viewRepetitions = 1;
	private static int recordingFrameLimit = 400;

	//Record file index
	public static int playbackFileNo;

	public static bool condition_for_creating_danceMoveFile = false;
	public static bool condition_for_creating_userFile = false;
	public static bool condition_for_writing_danceMoveFile = false;
	public static bool condition_for_writing_userFile = false;

	// stream used when recording to file
	public static StreamWriter streamWriter = null;

    // all the frames for avatars
    public static FrameData[] frames;
    public static int framesCount = 0;
    public static int currentFrame = 0;
    public static int framesRecorded = 0;

	// all the frames for skeleton displaying
	public static FrameData[] skel_frames;
	public static int skel_framesCount = 0;
	public static int skel_currentFrame = 0;

	//final dance
	public static bool danceUnlocked = false;

	public static bool raiseHandVoiceInfo = false;
	public static bool sayNextVoiceInfo = false;
	public static bool showFrameInfo = false;
	...
	protected static AvatarController instance = null;
	...
	public static FrameData? skel_getFrameData(StreamReader inputStream)
	{
		if (!inputStream.EndOfStream)
		{
			...
			Quaternion[] positionData = new Quaternion[FramesManager.no_skel];
			...
			try
			{
				...
				// read quaternions
				for (int i = 0; i < FramesManager.no_skel; i++)
				{
					line = inputStream.ReadLine();

					string[] lines = line.Split(',');
					x1 = int.Parse(lines[0]);
					y1 = int.Parse(lines[1]);
					x2 = int.Parse(lines[2]);
					y2 = int.Parse(lines[3]);
					positionData[i] = new Quaternion(x1, y1, x2, y2);
				}
			}
			...
			frameData.quaternions = positionData;
	...
	public static FrameData[] skel_loadFrames(string filename, ref int skel_framesCount)
	{
		skel_framesCount = 0;

		List<FrameData> skel_frames = new List<FrameData>();

		// get current file and open a new stream for it
		StreamReader streamReader = new StreamReader(filename, false);

		// get frames
		FrameData? newFrame = skel_getFrameData(streamReader);

		while (newFrame != null)
		{
			skel_frames.Add((FrameData)newFrame);
			skel_framesCount++;

			// next frame
			newFrame = skel_getFrameData(streamReader);
		}
	...
	public static FrameData? getFrameData(StreamReader inputStream)
	{
		if (!inputStream.EndOfStream)
		{
			...
			FrameData frameData = new FrameData();
			Quaternion[] quaternionData = new Quaternion[FramesManager.no_bones];
			...
			try
			{
				...
				// read quaternions
				for (int i = 0; i < FramesManager.no_bones; i++)
				{
	...
	/* close streams and reset variables */
	private void closeAndReset()
	{
		GameObject.Find("Canvas/RaiseHandImage").GetComponent<Image>().color = Color.red;
		GameObject.Find("Canvas/RaiseHandImage/Text").GetComponent<Text>().color = Color.white;
		...
		KinectManager.gestureDetected = false;
		noReps = 0;
		...
		if (raiseHandVoiceInfo)
		{
			NarrativeText.setText("Raise your right hand when you are ready to dance");
			NarrativeText.showInfoText("Say next or swipe left to see the next dance move", 5.0f);

			playbackFileNo = 0;
			playback_file = playback_directory + "Record0.txt";

			framesCount = 0;
			frames = loadFrames(playback_file, ref framesCount);

			raiseHandVoiceInfo = !raiseHandVoiceInfo;
		}
		...
		kinectManager.resetProgram();
	}

	public static Dictionary<string, bool> danceMoveInfo = new Dictionary<string, bool>();
	public static void initDanceMoveInfo()
	{
		danceMoveInfo["Record0.txt"] = false;
		danceMoveInfo["Record1.txt"] = false;
		danceMoveInfo["Record2.txt"] = false;
		danceMoveInfo["Record3.txt"] = false;
		danceMoveInfo["Record4.txt"] = false;
	}
	public static void setDanceMoveInfo(string ref_file)
	{
		danceMoveInfo[ref_file] = true;
	}

	public static bool checkDanceMoveInfo()
    {
		Dictionary<string, bool>.ValueCollection values = danceMoveInfo.Values;
	...
	public static void computeScore(string user_folder, string ref_file)
	{
		DTW dtw = new DTW(user_folder, ref_file);
		double resDTW = dtw.getCompResult();

		int feedback = (int)Math.Round(resDTW, 0);
		
		KinectManager.progressUI.SetFillerSizeAsPercentage(feedback);

		allowFeedback = true;

		if (feedbackVoiceInfoOnlyOnce)
		{
			NarrativeText.setTextDelayed("Raise your left hand for feedback", 1.5f);
			feedbackVoiceInfoOnlyOnce = false;
		}

		if (feedback < 30)
		{
			NarrativeText.setText("Try harder next time!");
		}
		else if (feedback >= 30 && feedback < 60)
		{
			NarrativeText.setText("You can do better");
		}
		else if (feedback >= 60)
		{
			if (feedback <= 80)
			{
				NarrativeText.setText("Good job!");
			}
			else
            {
				NarrativeText.setText("Excellent job!");
			}

			//mark dance move as completed
			string[] comp = ref_file.Split(Path.DirectorySeparatorChar);
			setDanceMoveInfo(comp[comp.Length - 1]);

			//check if all dance moves are completed(score > 60%)
			if(checkDanceMoveInfo())
            {
				danceUnlocked = true;

				NarrativeText.setTextDelayed("Congratulations! You can now dance Macarena", 1.5f);
				NarrativeText.showDanceMoveInfoText("Congratulations!", 3.0f);
				NarrativeText.showInfoText("Raise your right hand to start the final dance!", 8.0f);
			}
			else
            {
				NarrativeText.showInfoText("Raise your hand again for a retry or choose a different move", 5.0f);
			}
		}
	}
...
//            jointPosition.y - offsetPos.y + kinectManager.sensorHeight,  //!! this should be better investigated .. 
    ...
    public void UpdateAvatarFromKinect(Int64 UserID)
    {	
		// only update the user
		bool cond_for_update = ((KinectManager.gameModeStat == KinectManager.GameMode.Play) && !isTrainer) ||
			((KinectManager.gameModeStat == KinectManager.GameMode.Record) && isTrainer);
		if (!cond_for_update)
		{
			return;
		}

		if (condition_for_creating_danceMoveFile && (KinectManager.gameModeStat == KinectManager.GameMode.Record) && isTrainer)
		{
			...
			frames = null;
			framesRecorded = 0;
			Debug.Log(record_file + " was created.");

			condition_for_creating_danceMoveFile = false;
		}

		if(condition_for_creating_userFile)
        {
			if (streamWriter != null)
			{
				streamWriter.Close();
			...
			user_record_file = user_records_directory + "UserRecord" + noReps + ".txt";
			streamWriter = new StreamWriter(user_record_file, true);

			Debug.Log(user_record_file + " was created.");

			condition_for_creating_userFile = false;
		}
		...
		if (KinectManager.correctAvatarMovement)
		{
			// correct rotation
			//correctAvatarRotation();

			// set & correct body root position
			//correctBodyRootPos(frameData.rootPosition);
		}

		bool cond_for_writing = ((condition_for_writing_danceMoveFile && KinectManager.gameModeStat == KinectManager.GameMode.Record && isTrainer) || 
			condition_for_writing_userFile);
		if (cond_for_writing)
		{
			writeDataToFile(frameData);
		}
	...
	// manage speed for the current training program
	private bool manageSpeedMode()
	{
		if (KinectManager.speed == 0)
		{
			return false;
		}
		...
		{
			time = 0.03333f * (current_speed + 0.1f * KinectManager.speed);
			return false;
		}
	}

	public static void DrawSkeleton(Texture2D aTexture)
	{
		if (aTexture == null)
			return;

		if (skel_currentFrame < skel_framesCount)
		{
			FrameData? frameData = skel_frames[skel_currentFrame++];

			Quaternion[] positionData = frameData.Value.quaternions;

			for (var i = 0; i < FramesManager.no_skel; i++)
			{
				Quaternion quaternionData = positionData[i];

				int x1, y1, x2, y2;
				x1 = (int)quaternionData[0];
				y1 = (int)quaternionData[1];
				x2 = (int)quaternionData[2];
				y2 = (int)quaternionData[3];
				KinectInterop.DrawLine(aTexture, x1, y1, x2, y2, Color.yellow);
			}
		}
	}
	...
	{
		bool cond_for_update_trainer = ((KinectManager.gameModeStat == KinectManager.GameMode.View || 
			KinectManager.gameModeStat == KinectManager.GameMode.Play)
			&& isTrainer) 
			|| (KinectManager.gameModeStat == KinectManager.GameMode.Feedback && !isTrainer);
		if (!cond_for_update_trainer)
		{
			return;
		}
		...
		if (currentFrame < framesCount)
		{
			...
			if (showFrameInfo) //useful in viewmode debugging
			{
				Debug.Log("dance = " + currentFrame + "; skeleton = " + skel_currentFrame);
			}
		...
		else
		{
			...
			if (KinectManager.gameModeStat == KinectManager.GameMode.Play)
			{
				//final dance is still locked
				if (!danceUnlocked)
				{
					condition_for_creating_userFile = true;
					...
					if (noReps == maxRepetitions)
					{
						computeScore(user_records_directory, playback_file);
						closeAndReset();
					}
				}
				//all dance moves have been completed
				else
				{
					if (streamWriter != null)
					{
			...
			else if (KinectManager.gameModeStat == KinectManager.GameMode.View)
			{
				if (noReps == viewRepetitions)
				{
	...
	// apply correction such that the avatar will be facing the camera
	private void correctAvatarRotation()
	{
		Vector3 hipLeftPos = bones[17].position;
		Vector3 hipRightPos = bones[21].position;

		Vector3 dir = hipLeftPos - hipRightPos;

		float offset_angle = Mathf.Atan2(dir.z, dir.x) * 180 / Mathf.PI;

		//  rotate the bones hierarchy with the offset angle
		if (bones[0] != null)
			bones[0].Rotate(0, offset_angle, 0, Space.Self);
		else
			transform.Rotate(0, offset_angle, 0, Space.Self);
	}

	// set body root postion - apply correction s.t. the avatar's legs won't fall under (0, 0, 0)
	private void correctBodyRootPos(Vector3 rootPos)
	{
		if (bodyRoot != null)
			bodyRoot.position = rootPos;
		else
			transform.position = rootPos;

		float deltaY = (tipToesRight.transform.position.y + tipToesLeft.transform.position.y) / 2;

		if (deltaY < 0)
		{
			Debug.Log("Root DOWN correction");

			if (bodyRoot != null)
				bodyRoot.position = rootPos + new Vector3(0, Math.Abs(deltaY), 0);
			else
				transform.position = rootPos + new Vector3(0, Math.Abs(deltaY), 0);
		}
	}
